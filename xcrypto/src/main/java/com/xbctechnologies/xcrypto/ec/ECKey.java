package com.xbctechnologies.xcrypto.ec;
...
public class ECKey implements Serializable {
     ...
     */
    public ECKey(SecureRandom secureRandom, boolean iscompressed) {
        ...
        pub = CURVE.getCurve().decodePoint(pubParams.getQ().getEncoded(compressed));
    }

    protected ECKey(@Nullable BigInteger priv, ECPoint pub, boolean iscompressed) {
        compressed = iscompressed;
        this.priv = priv;
        if (pub == null)
    ...
    public ECPoint getPub(byte[] pubbytes) {
    ...
    public static ECKey fromPrivate(BigInteger privKey) {
        return new ECKey(privKey, compressPoint(CURVE.getG().multiply(privKey)), false);
    }
    ...
    public static ECKey fromPrivateAndPrecalculatedPublic(byte[] priv, byte[] pub) {
        ...
        return new ECKey(new BigInteger(1, priv), CURVE.getCurve().decodePoint(pub), false);
    }
    ...
    public static ECKey fromPublicOnly(ECPoint pub) {
        return new ECKey(null, pub, false);
    }
    ...
    public static ECKey fromPublicOnly(byte[] pub) {
        return new ECKey(null, CURVE.getCurve().decodePoint(pub), false);
    }
    ...
    public ECKey decompress() {
        ...
        if (compressed)
            ...
            return new ECKey(priv, decompressPoint(pub), compressed);
    }
    ...
    /**
     * Gets the Keccak256 form of the public key (as seen in addresses).
     */
    public byte[] getAddress() {
        if (pubKeyHash == null) {
            ...
            xbHashUtil.update(Arrays.copyOfRange(pubBytes, 1, pubBytes.length));
            byte[] hash = xbHashUtil.getHash();
            pubKeyHash = Arrays.copyOfRange(hash, 12, hash.length);
        }
        return pubKeyHash;
    }
    ...
    public String getPrivKeyHexString() {
        if (priv == null)
            throw new MissingPrivateKeyException();
        String keyhex = priv.toString(16);
    ...
    public static class ECDSASignature {
        ...
        public String toHex() {
            ...
            System.arraycopy(bigIntegerToBytes(this.r, 32), 0, sigData, 1, 32);
            System.arraycopy(bigIntegerToBytes(this.s, 32), 0, sigData, 33, 32);
            return Hex.toHexString(sigData);
        }
        ...
        public int getVInt() throws IllegalStateException {
            if (v >= 0) {
                Byte b = new Byte(v);
                int v = b.intValue();
                return v;
            } else {
                throw new IllegalStateException("wrong v value: " + v + " 0 or 1");
            }
        }
    ...
    public static ECKey signatureToKey(byte[] messageHash, String signatureBase64) throws SignatureException {
        ...
        return ECKey.fromPublicOnly(signatureToKeyBytes(
                messageHash,
     ...
     * @param signature signature.
     */
    public boolean verify(byte[] data, byte[] signature, boolean compressed) {
        return ECKey.verify(data, signature, getPubKey(compressed));
    }

    /**
     * Verifies the given R/S pair (signature) against a hash using the public key.
     */
    public boolean verify(byte[] sigHash, ECDSASignature signature, boolean compressed) {
        return ECKey.verify(sigHash, signature, getPubKey(compressed));
    }
    ...
    public boolean isPubKeyCanonical() {
        ...
        return isPubKeyCanonical(pub.getEncoded(compressed));
    }
    ...
    public int hashCode() {
        ...
        byte[] bits = getPubKey(compressed);
